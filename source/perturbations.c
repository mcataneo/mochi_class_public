    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
       ...
       */
        ...
        // dynamical smg variables for initial conditions
    ...
    if (ppt->pert_initial_conditions_smg == ext_field_attr){
      ...
      /*  We reach here if initialisation for a mode happens in quasi-static conditions.
          ...
          the amplitude at initialisation with that from inflation.
      ...
      */
          ...
          vxp_smg = 3./2.*(pow(2. - bra,2)*bra*pow(H,-2)*pow(M2,-1)*delta_rho_r +
                      ...
                      (2. - bra)*(cs2num - l2)*pow(M2*a,-1)*pow(H,-3)*rho_plus_p_theta/2. +
                      3./2.*(2. - bra)*((2. - bra)*(-7. + 2.*run)/4.*bra + 1./8.*bra*g3/g1 - l2 -
                      9./4.*(2. - bra)*bra*(p_tot + p_smg)*pow(H,-2) -
                      (1. - bra)*pow(a*H,-1)*bra_p)*pow(H,-2)*pow(M2,-1)*delta_p +
...
int perturb_einstein(
  ...
  double l1=0., l2=0., l3=0., l4=0., l5=0., l6=0., l7=0., l8=0., l9=0., l10=0., l2_p=0., l8_p=0., l9_p=0.;
  double M2=0., kin=0., bra=0., run=0., ten=0., bra_p=0., run_p=0., ten_p=0.;
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_smg == _TRUE_) {
        ...
        ten = ppw->pvecback[pba->index_bg_tensor_excess_smg];
        bra_p = ppw->pvecback[pba->index_bg_braiding_prime_smg];
        run_p = ppw->pvecback[pba->index_bg_mpl_running_prime_smg];
        ten_p = ppw->pvecback[pba->index_bg_tensor_excess_prime_smg];
        ...
        if (smgqs_array[ppw->approx[ppw->index_ap_smgqs]] == 0) {
          ...
          if ((pba->min_bra_smg<=2.) && (pba->max_bra_smg>=2.) || (ppr->get_h_from_trace==_TRUE_)) {
            ppw->pvecmetric[ppw->index_mt_vx_smg] = ((bra + 2.*run + (-2.)*ten + bra*ten)*(-2.)*pow(k,2)*M2*y[ppw->pv->index_pt_eta] + (-1.)*H*l2*M2*y[ppw->pv->index_pt_h_prime_from_trace_smg]*a + 9.*bra*ppw->delta_p*pow(a,2))*pow((bra + 2.*run + (-2.)*ten + bra*ten + l2)*(-2.)*H*pow(k,2)*M2*a + 2.*pow(H,3)*l9*M2*pow(a,3),-1);
          }
          else{
           ...
           * The result is approximated when rsa is on since the velocity of radiation gets updated only after the first Einstein equations (few lines below) */
           if ((pba->min_bra_smg<=2.) && (pba->max_bra_smg>=2.) || (ppr->get_h_from_trace==_TRUE_)) {
             ppw->pvecmetric[ppw->index_mt_vx_prime_smg] = (bra + 2.*run + (-2.)*ten + bra*ten)*(-3.)*pow(H,-1)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(M2,-1)*ppw->rho_plus_p_theta*a*pow((-2.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 2.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 4.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-2.)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_4_smg]*pow(a,2) + 2.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1) + (-3.)*bra*pow(H,-1)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(M2,-1)*ppw->rho_plus_p_theta_r*a*pow((-2.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 2.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 4.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-2.)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_4_smg]*pow(a,2) + 2.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1) + (3.*bra*rho_tot*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 6.*run*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-6.)*ten*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 3.*bra*ten*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-3.)*bra*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + pow(bra,2)*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + (-6.)*run*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + 2.*bra*run*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + 6.*ten*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + (-5.)*bra*ten*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_rho_tot_wo_smg]*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + (-3.)*bra*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + pow(bra,2)*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + (-6.)*run*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + 2.*bra*run*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + 6.*ten*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + (-5.)*bra*ten*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + pow(bra,2)*ten*rho_smg*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2 + 2.*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_5_smg]*M2 + 3.*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + 6.*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + (-6.)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + 3.*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + (-3.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + (-6.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + 6.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + (-3.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_p_tot_wo_smg] + (-3.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*p_smg + (-6.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*p_smg + 6.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*p_smg + (-3.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*p_smg + 6.*pow(H,4)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_6_smg]*M2*pow(a,2))*(-1.)*pow(H,-1)*pow(M2,-1)*ppw->pvecmetric[ppw->index_mt_vx_smg]*a*pow((-2.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 2.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 4.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-2.)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_4_smg]*pow(a,2) + 2.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1) + ((-4.)*run*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*bra_p + 4.*ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*bra_p + (-2.)*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*bra_p + 4.*bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*run_p + (-4.)*bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ten_p + 2.*pow(bra,2)*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ten_p + 2.*bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + 3.*pow(bra,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 6.*bra*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 2.*pow(bra,2)*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 4.*bra*pow(run,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-6.)*bra*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 3.*pow(bra,2)*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-4.)*bra*run*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 2.*pow(bra,2)*run*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-2.)*bra*kin*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + (-4.)*kin*run*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 4.*kin*ten*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + (-2.)*bra*kin*ten*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 3.*bra*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 2.*bra*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + (-2.)*kin*pow(H,2)*pow(k,2)*pow(ppw->pvecback[pba->index_bg_lambda_2_smg],2)*a + (-3.)*pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-6.)*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 6.*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-3.)*pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-3.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-3.)*pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + (-6.)*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 6.*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + (-3.)*pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + (-3.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*p_smg*a + 2.*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*bra_p*pow(a,2) + (-2.)*bra*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_prime_smg]*pow(a,2) + (-1.)*bra*pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + (-2.)*bra*run*pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + 2.*kin*pow(H,4)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + 9.*bra*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*pow(a,3) + 9.*bra*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*p_smg*pow(a,3))*(-3.)/2.*pow(H,-2)*pow(M2,-1)*ppw->delta_rho_r*a*pow(bra*pow(k,2) + 2.*run*pow(k,2) + (-2.)*ten*pow(k,2) + bra*ten*pow(k,2) + pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + (-1.)*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1)*pow((-2.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 2.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 4.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-2.)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_4_smg]*pow(a,2) + 2.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1) + (2.*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*bra_p + 2.*ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*bra_p + 4.*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*run_p + (-4.)*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ten_p + 2.*bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ten_p + (-2.)*bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + (-4.)*run*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + 4.*ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + (-2.)*bra*ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + pow(bra,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 4.*bra*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 4.*pow(run,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-4.)*bra*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 2.*pow(bra,2)*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-8.)*run*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 4.*bra*run*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 4.*pow(ten,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-4.)*bra*pow(ten,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + pow(bra,2)*pow(ten,2)*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + bra*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 2.*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + (-2.)*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + bra*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 2.*bra*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_1_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 4.*run*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_1_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + (-4.)*ten*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_1_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 2.*bra*ten*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_1_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + 2.*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_1_smg]*pow(ppw->pvecback[pba->index_bg_lambda_2_smg],2)*a + 3.*pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 12.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 12.*pow(run,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-12.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 6.*pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-24.)*run*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 12.*bra*run*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 12.*pow(ten,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-12.)*bra*pow(ten,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 3.*pow(bra,2)*pow(ten,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 3.*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 6.*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + (-6.)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 3.*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*a + 3.*pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 12.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 12.*pow(run,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + (-12.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 6.*pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + (-24.)*run*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 12.*bra*run*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 12.*pow(ten,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + (-12.)*bra*pow(ten,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 3.*pow(bra,2)*pow(ten,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*p_smg*a + 3.*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*p_smg*a + 6.*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*p_smg*a + (-6.)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*p_smg*a + 3.*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*p_smg*a + (-2.)*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*bra_p*pow(a,2) + (-2.)*ten*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*bra_p*pow(a,2) + (-4.)*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*run_p*pow(a,2) + 4.*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ten_p*pow(a,2) + (-2.)*bra*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ten_p*pow(a,2) + 2.*bra*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_prime_smg]*pow(a,2) + 4.*run*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_prime_smg]*pow(a,2) + (-4.)*ten*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_prime_smg]*pow(a,2) + 2.*bra*ten*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_prime_smg]*pow(a,2) + (-3.)*bra*pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + (-6.)*run*pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + 6.*ten*pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + (-3.)*bra*ten*pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + (-2.)*pow(H,4)*ppw->pvecback[pba->index_bg_lambda_1_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + (-9.)*bra*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*pow(a,3) + (-18.)*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*pow(a,3) + 18.*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*pow(a,3) + (-9.)*bra*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*pow(a,3) + (-9.)*bra*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*p_smg*pow(a,3) + (-18.)*run*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*p_smg*pow(a,3) + 18.*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*p_smg*pow(a,3) + (-9.)*bra*ten*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*p_smg*pow(a,3))*(-1.)*pow(H,-2)*pow(k,2)*y[ppw->pv->index_pt_eta]*pow(a,-1)*pow(bra*pow(k,2) + 2.*run*pow(k,2) + (-2.)*ten*pow(k,2) + bra*ten*pow(k,2) + pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + (-1.)*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1)*pow((-2.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 2.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 4.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-2.)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_4_smg]*pow(a,2) + 2.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1) + (H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*M2*bra_p + ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*M2*bra_p + 2.*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*M2*run_p + (-2.)*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*M2*ten_p + bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*M2*ten_p + (-1.)*bra*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + (-2.)*run*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + 2.*ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + (-1.)*bra*ten*H*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*M2*ppw->pvecback[pba->index_bg_lambda_2_prime_smg] + (-2.)*pow(bra,2)*rho_r*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-4.)*bra*run*rho_r*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + 4.*bra*ten*rho_r*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-2.)*pow(bra,2)*ten*rho_r*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*a + (-2.)*bra*rho_r*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*a + (-2.)*bra*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_3_smg]*M2*a + (-4.)*run*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_3_smg]*M2*a + 4.*ten*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_3_smg]*M2*a + (-2.)*bra*ten*pow(H,2)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_3_smg]*M2*a + (-2.)*pow(H,2)*pow(k,2)*pow(ppw->pvecback[pba->index_bg_lambda_2_smg],2)*ppw->pvecback[pba->index_bg_lambda_3_smg]*M2*a + pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*M2*ppw->pvecback[pba->index_bg_lambda_2_prime_smg]*pow(a,2) + (-1.)*pow(H,3)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*M2*ppw->pvecback[pba->index_bg_lambda_9_prime_smg]*pow(a,2) + 2.*bra*rho_r*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,3) + pow(H,4)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*M2*pow(a,3) + 2.*pow(H,4)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_3_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*M2*pow(a,3) + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*M2*ppw->pvecback[pba->index_bg_p_tot_wo_smg]*pow(a,3) + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*M2*p_smg*pow(a,3))*(-1.)*pow(H,-1)*pow(M2,-1)*ppw->pvecmetric[ppw->index_mt_h_prime]*pow((-1.)*bra*pow(k,2) + (-2.)*run*pow(k,2) + 2.*ten*pow(k,2) + (-1.)*bra*ten*pow(k,2) + (-1.)*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + pow(H,2)*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1)*pow((-2.)*bra*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 2.*bra*run*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + 4.*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-4.)*bra*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + pow(bra,2)*ten*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2) + (-2.)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*pow(k,2)*ppw->pvecback[pba->index_bg_lambda_2_smg] + 3.*pow(H,2)*ppw->pvecback[pba->index_bg_lambda_2_smg]*ppw->pvecback[pba->index_bg_lambda_4_smg]*pow(a,2) + 2.*pow(H,2)*ppw->pvecback[pba->index_bg_kinetic_D_smg]*ppw->pvecback[pba->index_bg_lambda_9_smg]*pow(a,2),-1);
           }
           else{
