...
int background_gravity_functions(
  ...
  if (pba->field_evolution_smg == _TRUE_) {
    ...
    if (pba->gravity_model_smg == quintessence_monomial) {
    ...
    else if(pba->gravity_model_smg == low_order_G){
      ...
      */
      ...
      double c01 = pba->parameters_smg[0];
      double c20 = pba->parameters_smg[1]*pow(pba->H0,2.);
      double d01 = pba->parameters_smg[2]/pow(pba->H0,2.);
      double e20 = pba->parameters_smg[3];

      G2 = c01*X + c20*pow(phi,2.);
      G2_X = c01;
      G2_phi = 2.*c20*phi;
    ...
    else if(pba->gravity_model_smg == general_G){; // c01, c02, c11, c20, d01, d02, d11, e10, e20, phi_ini, phi_ini_prime

      /* Action is

        G2 = c01*X + c02*X^2 + c11*phi*X + c20*phi^2
        G3 = - (d01*X + d02*X^2 + d11*phi*X)
        G4 = 1/2 + e01*X + e20*phi^2

      */

      double c01 = pba->parameters_smg[0];
      double c02 = pba->parameters_smg[1]/pow(pba->H0,2.);
      double c11 = pba->parameters_smg[2];
      double c20 = pba->parameters_smg[3]*pow(pba->H0,2.);
      double d01 = pba->parameters_smg[4]/pow(pba->H0,2.);
      double d02 = pba->parameters_smg[5]/pow(pba->H0,4.);
      double d11 = pba->parameters_smg[6]/pow(pba->H0,2.);
      double e01 = pba->parameters_smg[7]/pow(pba->H0,2.);
      double e20 = pba->parameters_smg[8];

      G2 = c01*X + c02*pow(X,2.) + c11*phi*X + c20*pow(phi,2.);
      G2_X = c01 + 2.*c02*X + c11*phi;
      G2_XX = 2.*c02;
      G2_phi = c11*X + 2.*c20*phi;
      G2_Xphi = c11;

      // G3 = - (d01*X + d02*pow(X,2.) + d11*phi*X)
      G3_X = - (d01 + 2.*d02*X + d11*phi);
      G3_XX = - 2.*d02;
      G3_phi = - d11*X;
      G3_Xphi = - d11;

      G4_smg = e01*X + e20*pow(phi,2);
      G4 = 1/2. + G4_smg;
      G4_phi = 2.*e20*phi;
      G4_phiphi = 2.*e20;
      G4_X = e01;
    ...
    else if(pba->gravity_model_smg == full_bl_G){; //c01, c02, c11, c20, d01, d02, d11, e01, e02, e11, e20, f01, f02, f11,  phi_ini, phi_ini_prime

      /*Action is
	G2 = c01*X + c02*X^2 + c11*phi*X + c20*phi^2
        G3 = - (d01*X + d02*X^2 + d11*phi*X)
        G4 = 1/2 + e01*X + e02*X^2 + e11*phi*X + e20*phi^2
        G5 = f01*X + f02*X^2 + f11*phi
      */
      ...
      double e01 = pba->parameters_smg[7]/pow(pba->H0,2.);
      double e02 = pba->parameters_smg[8]/pow(pba->H0,4.);
      double e11 = pba->parameters_smg[9]/pow(pba->H0,2.);
      double e20 = pba->parameters_smg[10];

      double f01 = pba->parameters_smg[11]/pow(pba->H0,2.);
      double f02 = pba->parameters_smg[12]/pow(pba->H0,4.);
      double f11 = pba->parameters_smg[13]/pow(pba->H0,2.);
      ...
      G4_smg = e01*X + e02*pow(X,2.) + e11*phi*X + e20*pow(phi,2);
      G4 = 1/2. + G4_smg;
      G4_phi = e11*X + 2.*e20*phi;
      G4_phiphi = 2.*e20;
      G4_X = e01 + 2.*e20*X + e11*phi;
      G4_XX = 2.*e20;
      G4_Xphi = e11;

      G5 = f01*X + f02*pow(X,2.) + f11*phi*X;
      G5_phi = f11*X;
      G5_X = f01 + 2.*f02*X + f11*phi;
      G5_XX = 2.*f02;
      G5_Xphi = f11;
    }
