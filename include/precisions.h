...
class_precision_parameter(tol_background_bw_integration,double,1.e-10) /**< Tolerance of the backward background integration for braiding, giving the allowed relative integration error -- 1st order ODE */
class_precision_parameter(dtau_start_qs,double,1e-7) /**< enforce conformal time delay to switch from FD to QS such that scalar field ICs can be properly set up with stable_params */
class_precision_parameter(braid_activation_threshold,double,1e-12) /**< smallest value for braiding above which smg is activated (assuming smg parameters are decaying to zero as power laws at high redshift) */
